...
module ApiAuth
  RSpec.describe ClientsController, type: :controller do
    ...
    describe 'Toggle api' do
      it 'toggle api allow PUT' do
        client = create(:client)
        api = create(:api)

        expect {
          put :toggle_api, params: { id: client.id, api: { id: api.id, 'PUT' => 'true' } }
        }.to change { client.authorized_apis.reload.size }.by(1)
        expect(client.authorized_apis.reload.last).to be_PUT
      end

      it 'toggle api disable PUT' do
        client = create(:client)
        api = create(:api)
        put :toggle_api, params: { id: client.id, api: { id: api.id, 'PUT' => 'true' } }

        expect {
          put :toggle_api, params: { id: client.id, api: { id: api.id, 'PUT' => 'false' } }
        }.to change { client.authorized_apis.size }.by(0)
        expect(client.authorized_apis.last.PUT).to be(false)
      end
    end
  end
end
